    ...
    EP: EdgePotential<Value = X::State>,
{
    ...
    pub fn belief_propagation(&self) -> Array2<f64> {
        ...
        let mut log_messages: HashMap<(NodeIndex, NodeIndex), Vec<f64>> = HashMap::new();//MessageBank::new(X::size(), num_nodes, 0.0);//
                    ...
                    for (j, xj) in X::states().enumerate() {
                        for (i, xi) in X::states().enumerate() {
                            let log_message_from_neighbors: f64 =  self
                                ...
                                .map(|k| log_messages.entry((k, m)).or_insert(vec![0.0; X::size()])[i]) // Get the value of the message for value xi, from k to i
                        ...
                        log_messages.entry((m, n)).or_insert(vec![0.0; X::size()])[j] = logsumexp(log_message_container.as_slice());
